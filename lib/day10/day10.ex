defmodule Day10 do
  ...
  def fp_simulation(inslist, cv, cycle, ci, citimer, state, ap) when ap == false do
    if cycle < 221 and rem(cycle + 20, 40) == 0 do
      fp_simulation(inslist, cv, cycle, ci, citimer, Map.put(state, cycle, cv), true)
    else
      fp_simulation(inslist, cv, cycle, ci, citimer, state, true)
    end
  end

  def fp_simulation(inslist, cv, cycle, ci, citimer, state, _ap) when citimer != 0 do
    fp_simulation(inslist, cv, cycle + 1, ci, citimer - 1, state, false)
  end

  def fp_simulation(inslist, cv, cycle, ci, _citimer, state, _ap) do
    ncv =
      case ci do
        ["noop"] -> cv
    ...
    if tl(inslist) == [] do
      ...
      fp_simulation(tl(inslist), ncv, cycle + 1, nci, ncitimer - 1, state, false)
    end
  end

  def part_one() do
    input()
    |> fp_simulation(1, 0, ["noop"], 0, Map.new(), false)
